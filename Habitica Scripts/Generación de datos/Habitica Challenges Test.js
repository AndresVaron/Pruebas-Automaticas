...
context('Habitica Tests', () => {
  ...
  it('register a new challenge', () => {
    ...
    cy.wait(3000)
    cy.get('input[placeholder="What is your Challenge name?"]').type(cy.faker.name.findName())
    cy.get('input[placeholder="What short tag should be used to identify your Challenge?"]').type(cy.faker.lorem.word())
    cy.get('.summary-textarea').type(cy.faker.lorem.words())
    cy.get('.description-textarea').type(cy.faker.lorem.sentence())
