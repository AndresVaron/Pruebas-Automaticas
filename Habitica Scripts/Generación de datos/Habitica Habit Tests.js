...
context('Habitica Habit Tests', () => {
  ...
  it('edit habit', () => {
      cy.wait(1000)
      cy.get('.habit > .tasks-list > .sortable-tasks > :nth-child(1) > .task > :nth-child(1) > .task-content > .task-clickable-area').click();
      cy.wait(2000)
    ...
    it('get monay with habit', () => {
      cy.get('.habit > .tasks-list > .sortable-tasks > :nth-child(1) > .task > :nth-child(1) > .left-control > .task-control').click({force: true})
      cy.wait(1000)
      cy.get(':nth-child(1) > .notification > .row > .text > div').should('contain', 'You gained some Gold')
    }) 

    it('loss life with habit', () => {
      cy.get(':nth-child(1) > .task > :nth-child(1) > .right-control > .task-control').click()
      cy.wait(1000)
