...
context('Habitica Tests', () => {
  ...
  it('makes a sign up attemp with password missmatch', () => {
    ...
    cy.get('[type="email"]').type(cy.faker.internet.email())
    cy.get('[placeholder="Password"]').type(cy.faker.internet.password())
    cy.get('[placeholder="Confirm Password"]').type(cy.faker.internet.password())
