...
const PostUploadFile = async (req, res, next) => {
    try {
        ...
        if (!filePath || filePath.includes('..')) {
            return res
                .status(400)
                .json({ message: 'No se definió la ruta del archivo' });
        }
        const fileToUpload = req.files.file;
        if (!fileToUpload) {
            return res
                .status(400)
                .json({ message: 'No se seleccionó ningún archivo' });
        }
        ...
        const fileToUploadName = `${filePath}/${
            fileName || new Date().getTime()
        }${path.extname(fileToUpload.name)}`;

        uploadFile(req.files.file.data, fileToUploadName)
            .then((data) => {
                console.log(data);
                res.json({
                    data: `https://${bucketName}.s3.amazonaws.com/${fileToUploadName}`,
                });
            })
            .catch((err) => {
                res.status(400).json({
