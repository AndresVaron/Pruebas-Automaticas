...
module.exports.postWebAppVersionConfig = async (id_version) => {
    ...
    try {
        ...
        const response = await WebAppPersistence.insertWebAppConfig(
            finalWebAppConfig
        );
        return response.ops[0];
    } catch (err) {
...
module.exports.getWebAppVersionConfig = async (
    ...
    try {
        const response = await WebAppPersistence.fetchWebAppVersionConfig(
            id_config
        );
        if (response === null) {
            const errJson = {
...
module.exports.updateWebAppVersionConfig = async (id, config) => {
    ...
    try {
        ...
        updateResponse = await WebAppPersistence.updateWebAppConfig(id, config);
    } catch (err) {
        if (err.errCode) {
            throw err;
        }
...
module.exports.deleteWebAppConfig = async (id) => {
    try {
        const response = await WebAppPersistence.deleteWebAppConfig(id);
        //Se revisa si existia una web app con ese id
        if (response.result.n == 1) {
            return response;
        } else {
