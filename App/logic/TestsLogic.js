...
const postTest = async (id, { name, shortName, url, version }) => {
    ...
    try {
        ...
        if (aut.length === 0) {
            ...
            const newTest = { name, shortName, creationDate: new Date(), aut: aut[0]._id, versions: [] };
            const createdTest = await insertTest(newTest);
            const createdVersion = await insertVersion({ test: createdTest.ops[0]._id, creationDate: new Date(), version, url });
