...
const executeVRT = (test, directory, packageName, deviceName, appVersion, mobile = true) => {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (mobile) {
                ...
                const results = await cypress.run({ spec: directory + '/' + packageName + '.js' });
                console.log(results);
                directory = directory.replace('integration', 'screenshots') + '/' + packageName + '.js';
            }
            const files = readdirSync(directory);
            const fileToUploadName = `${mobile ? 'mobile' : 'web'}/vrt/${appVersion.id_app}/version/${appVersion._id}/test/${test.test}/version/${test._id}/`;
            const uploaded = [];
            ...
            if (tests && tests.length > 0) {
                ...
                await insertTestResult({ appVersion: appVersion._id, aut: appVersion.id_app, test: test.test, type: 'VRT', mobile, creationDate: new Date(), testVersion: test._id, images: uploaded, numberOfImages: uploaded.length })
            }
            resolve(true);
        } catch (error) {
            console.log(error);
