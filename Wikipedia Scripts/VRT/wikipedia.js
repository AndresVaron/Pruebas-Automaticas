...
//Se hace esta función con los pasos comunes para ir atrás.
const goToHome = async () => {
    const buttonMore = await client.$('android=new UiSelector().description("Navigate up")');
    await buttonMore.click();
}

//Se hace esta función con los pasos comunes para ir a un articulo en el listado de guardados.
...
//Se hace esta función con los pasos comunes para dar en el boto de no en el mensaje de configuración de autofill.
const noSettingsAutoFill = async () => {
    try {
        const noButton = await client.$('android=new UiSelector().resourceId("android:id/button2")');
        await noButton.click();
    } catch (error) {
        console.log(error);
    }
...
//Se hace esta función con los pasos para buscar articulo en la app.
const searchArticles = async () => {
    ...
    //Se maneja este caso el catch porque algunas ocasiones la app despliega esta opción como Home y otras veces como Explore
    } catch (error) {
        console.log(error);
        const homeButton2 = await client.$('android=new UiSelector().text("Explore")');
...
//Se hace esta función con los pasos para diligenciar de una manera correcta el usuario y contraseña
const logIn = async () => {
...
//Se hace esta función con los pasos para ver la opción de editar, como no se tienen articulos para editar se deja solo en la vista principal de esa funcionalidad.
const editing = async () => {
...
//Se hace esta función con los pasos para ver la sincronización de los articulos con el usuario logueado, lista guardados para este caso.
...
//Se hace esta función con los pasos compartir un articulo desde la app.
